...
export type CommandCategory = 'file_management' | 'process_control' | 'network' | 'development' | 'git' | 'docker' | 'kubernetes' | 'database' | 'monitoring' | 'security';
...
export interface TerminalAPIClient {
    ...
    analyzeUsage(session_id: string, period?: {
        start: string;
        end: string;
...
export type TerminalEventType = 'session_created' | 'session_terminated' | 'command_started' | 'command_completed' | 'output_received' | 'error_occurred' | 'workflow_started' | 'workflow_completed' | 'ai_suggestion' | 'safety_warning';
