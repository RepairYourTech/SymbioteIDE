...
The cleanup that motivated this tool removed 57 clean worktrees and 70 local branches whose content was in `main`; the worktrees alone held about 140 GiB of build caches, and branches pointing at superseded heads produced diffs that read like uncommitted work. Those are that cleanup's own numbers, from the checkout it ran in: no case reads this file, so nothing here re-measures them. This tool is the repeatable form of that cleanup: it never pushes, never rewrites history, and mutates nothing outside the local repository.
