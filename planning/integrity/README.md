...
The second half covers the inputs cargo reads rather than rustc: the record must also name the workspace manifest, its lockfile and the manifest of every package in the binary's closure. No per-unit dep-info names them — measured on a build-only target directory, the rustc oracle names none of them, so dropping one from the record passed this check before this half existed — so they are taken from `cargo metadata` instead. The workspace manifest is the sharp case rather than a formality: a member says `edition.workspace = true`, so its content decides the edition every member compiles under while every `.rs` file stays byte-identical, and a bump of `version` is refused by nothing else, since no source in this tree reads `CARGO_PKG_VERSION`.
