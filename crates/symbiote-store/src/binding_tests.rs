...
fn v4_binding_migration_preserves_history_and_rolls_back_corrupt_input() {
    for corrupt in [false, true] {
        ...
        store
            .connection
            .execute_batch(
                "DROP TABLE model_descriptors; DROP TABLE billing_entitlements; DROP TABLE dispatch_preparations; DROP TABLE provider_connections; DROP TABLE task_leases; DROP TABLE task_dependencies; DROP TABLE work_routes; DROP TABLE workforce_bindings; PRAGMA user_version=4;",
            )
            .unwrap();
        if corrupt {
            store
