...
impl Store {
    ...
    fn initialize(mut connection: Connection) -> Result<Self> {
        ...
        connection.busy_timeout(Duration::from_secs(WRITER_BUSY_TIMEOUT_SECS))?;
...
/// The busy timeout the writer sets, the figure `storage.md` states. One owner: the contract case
/// below reads this declaration, and the connection configuration above applies it.
const WRITER_BUSY_TIMEOUT_SECS: u64 = 5;
...
/// The busy timeout `storage.md` states is the one this crate's writer sets. The figure is read
/// from the sentence it is written in and the timeout from the declaration the connection is
/// configured with, so a document that states a timeout this crate has moved past fails here by
/// name rather than in prose nobody reads.
///
/// What it does not read: the prose around it — `BEGIN IMMEDIATE`, WAL, `synchronous=FULL`,
/// the revision-conflict and rollback behaviour — which the crate's own cases drive against real
/// databases.
///
/// The case sits at the crate root of the library rather than in `tests/` because the writer's
/// timeout is a crate-private fact: it is named once, applied where the connection is configured,
/// and read here from the declaration instead of out of this file's text, while the test name the
/// census records stays the one it was.
#[cfg(test)]
#[test]
fn the_contract_states_the_writer_timeout_this_crate_sets() {
    ...
    assert_eq!(
        stated, WRITER_BUSY_TIMEOUT_SECS,
        "the contract states a {stated}-second busy timeout, and this crate sets {WRITER_BUSY_TIMEOUT_SECS}"
    );
}
