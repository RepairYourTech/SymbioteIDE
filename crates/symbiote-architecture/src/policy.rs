//! The decision lifecycle this repository enforces, in memory (#173).
//!
//! States, authority, evidence, version pins, compatibility facts and the
//! registry that owns them: one concern — what this repository is allowed to
//! conclude — and no file in sight. [`DecisionRegistry`] is the single owner of
//! every decision, fact, pin and approval, and every transition validates
//! completely before it mutates, so a caller can never hold a state these rules
//! do not produce.
//!
//! The repository's own data is `crate::ledger`, which replays the committed
//! file through these transitions; what the tree must show for what comes out is
//! `crate::checks`. Nothing here reads a file or a clock, so the Host — which
//! must do both — authenticates itself and its inputs before it calls in.
...
/// The name this repository publishes for a state. One owner for the vocabulary
/// the ledger, the refusals and the published map all print; the suite holds it
/// to the serialized name, so a renamed variant cannot be printed one way and
/// stored another.
...
/// The name this repository publishes for a gate status, owned where the status
/// is, for the same reason [`DecisionState`]'s name is.
impl std::fmt::Display for GateStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let name = match self {
            GateStatus::Settled => "settled",
            GateStatus::Provisional => "provisional",
