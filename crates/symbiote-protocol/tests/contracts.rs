...
#[test]
fn the_projection_read_is_a_project_read_and_not_owner_authority() {
    ...
    assert!(
        authorize(
            &reader,
            &request(Operation::GetSchedulingProjection {
                project_id: project_id(),
            }),
        )
        .is_ok(),
        "a Project read is authority to read that Project's scheduling projection"
    );
    // Another Project is not this grant's business, and no other
    ...
    for permission in [
        ...
        assert_eq!(
            ...
            .code,
            ErrorCode::PermissionDenied,
            "{permission:?} is not authority to read the projection"
        );
    }
    // The wire carries the Project and nothing else: no actor, no percentage and
    // no field that could move a Task.
    let encoded = serde_json::to_value(Operation::GetSchedulingProjection {
        project_id: project_id(),
    })
    .unwrap();
    assert_eq!(
        encoded,
        json!({"kind": "get_scheduling_projection", "project_id": "project-a"})
    );
    ...
    assert!(
        negotiate(&[CURRENT_VERSION])
            .unwrap()
            .capabilities
            .contains(&symbiote_protocol::Capability::SchedulingProjection)
    );
}
...
fn golden_request_and_response_remain_stable() {
    let fixture = r#"{"version":{"major":1,"minor":29},"correlation_id":"request-a","command_id":"command-a","operation":{"kind":"get_project","project_id":"project-a"}}"#;
    ...
    assert_eq!(
        ...
        json!({"version":{"major":1,"minor": 29},"correlation_id":"request-a","result":{"Err":{"code":"not_found","message":"resource not found"}}})
    );
}
