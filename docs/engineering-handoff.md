...
A decision can now publish `accepted` only while a complete, in-threshold run of its contract stands, cited as its own evidence: `spike.rs` reads the run (contract identity, the contract's fingerprint when it ran, outcome, untested platforms, one observation per predeclared measurement, and the raw artifacts it published with their SHA-256) and `checks.rs` joins it to the tree and the ledger. The fingerprint is what makes the ceilings predeclared rather than chosen after the fact: a threshold cannot move under a result measured against it. With no run committed, the shell choice stays `investigating`, which is the honest state — nothing has been measured.
