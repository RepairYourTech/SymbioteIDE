...
The tie runs the other way too, so a record cannot arrive ownerless. Four rules close the round trip, each refused by name: every canonical type must be indexed in the artifact and nothing else may be; an indexed type must be owned by exactly one noun, or be an identity-only type excused with its reason — `HostId` and `CredentialReferenceId`, the identities the `host` and `credential reference` nouns name, are the whole of that list, and it is checked in both directions (an entry whose name is gone fails, one that owns a noun fails, one that is no noun's identity fails); every record the envelope carries must be a canonical type whose noun is an **entity**, since a value object is identified only through its carrier and cannot stand as a record; and every published name must be either an indexed canonical type or a definition one of them refers to. The entity nouns' owners and the envelope's records are therefore the same set — asserted as an equality, not left to the two rules — so a type added on one side alone fails whether it arrives in the vocabulary, in `DomainRecord`, or in the artifact.
...
Two boundaries are stated where a reader meets them rather than left to read as more than they are. Nineteen records carry no shared `Provenance`: eighteen predate this change and keep their lineage in record-specific fields (`Task.history`, `VerificationEvidence.verified_by`, `WorkforceRuntimeContract.compiled_at` and so on), and `CredentialReference` holds an identity and a vault locator with no lineage to record. The suite lists them exactly, fails if a record gains shared lineage while still listed, and routes unifying them to the persistent store that decides the surviving record shape. `WorkforceRuntimeContract` is the one record whose identity is its owning Dispatch's `contract_id` rather than a field of its own, because it is immutable-per-Dispatch compiled state; `CredentialReference` is an entity for the same reason it is a record — it carries identity of its own — which is why `credential reference` is classified as an entity rather than a value object.
