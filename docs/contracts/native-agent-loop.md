...
3. Validates the response envelope (request/provider/model identity), accumulates usage conservatively (a total without detail counts entirely against the output remainder, so the check can never overspend the contract), records SDK `RuntimeEvent`s (`Message`, `ToolProposed`, `ToolFailed`, `Diagnostic`, `Exit`, `CancelAcknowledged`) with the session binding, and continues on `ToolCalls`.
...
No HTTPS transport yet: every test runs against offline envelopes; **live Responses-API verification requires user authorization for credentials and billing, and none was performed**. Tools are validated and recorded but not executed in the sandbox (next slice: sandbox invocation through #218's launcher with the #211 reserved worktree). No live process containment, no streaming, no resume-after-crash of an in-flight turn sequence. External (Codex) execution shares none of this code path and cannot bill native credentials.

## Evidence

Five offline tests: single-turn stop within contract budget (request honors contract context, not model window; tools exactly the contract's), tool-calls continue until stop + undeclared-tool halt, budget exhaustion refusing before the provider call, completion-report-as-evidence with double-filing refusal, and expired-contract session refusal. A daemon test proves the completion request against an unstarted task fails closed and the task stays Ready.
