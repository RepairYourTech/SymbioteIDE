...
command (`Err` response) — scripts branch on refusal vs transport failure.
`help` lists the command table. Identity arguments are passed as plain
strings and typed on the wire; no caller-supplied actor identities exist.
The interactive/headless coding-agent experience is #467's surface and
