...
| `checks/contracts.rs` | what the contracts and their runs must show — the contract/decision link, obligation ownership, the accepted bar read from the record and every clause of it answered, and the run set a settled choice stands on |
...
The bar is not the contract's own statement of it. `SpikeContract.obligations` names the section of the decision's accepted record — the text the ledger pins by SHA-256 — that states what a proof of that choice must do, and answers every clause of it: with an obligation this contract declares and a run has to exercise, with `method` or `measurements` where the clause is about how the proof is run rather than what it exercises, or with an `elsewhere` naming the issue that owns the clause where it belongs to the decision's wider acceptance. The clauses are the record's own, split at its sentence and semicolon boundaries, so nothing is quoted or restated here — the accepted text stays the one readable copy, and this says what answers each clause of it. Every obligation the contract declares must answer a clause too, so the contract can neither shrink the accepted bar nor widen it.
...
| a `section` the decision's accepted record does not state, or an answer whose clause number is past the clauses the section states | a bar read from a heading nobody wrote, or from a clause the record does not state, is not the accepted bar |
| a clause of the accepted bar that nothing answers, an answer naming an obligation the contract does not declare, an answer naming two carriers at once, or an `elsewhere` naming an issue the choice does not own | the bar is the accepted record's, so dropping an obligation from the contract, or pointing a clause at something the contract does not hold, is refused rather than absorbed |
