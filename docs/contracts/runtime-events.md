...
These are SDK contract tests, not native-agent, Codex, PTY or process-cleanup evidence. The native production adapter separately binds every event to the complete `SessionBinding` and the live session before returning it, but its `MockHarness` is not process evidence. #184 remains open for concrete process-backed harness integration, the remaining lifecycle methods and capability tiers, qualified tool enforcement, authenticated/redacted raw-event handling, continuation/checkpoint persistence, rate-limit/quota reporting, advanced child lineage/cost coverage and broader platform-specific compatibility dossiers. No canonical Task mutation or actual runtime execution is enabled by this module.
