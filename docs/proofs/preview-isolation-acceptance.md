...
5. **The ACL rejection, live and in-context** (the #524 review's explicitly deferred verification): the Preview window's isolation self-check — an owner-injected constant probe, evaluated in the preview's own context after page load — attempted the read-only owner command `journal_position` **from the preview window** and the app ACL rejected it. The page records the platform's own diagnostic: `Isolation self-check: OK: app command rejected by the app ACL (journal_position not allowed on window "preview", webview "preview", URL: local allowed on: [windows: "main", URL: local] referenced by: capability: default, permission: allow-journal-position)`. The probe is a constant of the binary — worker content never reaches it — and it runs on every page load, so the surface re-attests its isolation on every open, visible in the window title and page body.
...
# then dump the preview frame's text and check:
