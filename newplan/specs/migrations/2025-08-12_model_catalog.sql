-- Symbiote Model Catalog: Supabase source-of-truth schema
-- Run this in Supabase SQL editor for project: SymbioteIDE (apgzyopdlcpwjjyebgtm)
...
create trigger if not exists models_set_updated_at
...
create trigger if not exists model_ratings_set_updated_at
...
create trigger if not exists model_combinations_set_updated_at
...
create trigger if not exists user_model_preferences_set_updated_at
...
-- Public SELECT (adjust later for auth-only)
create policy if not exists models_select_anon on public.models for select using (true);
create policy if not exists task_categories_select_anon on public.task_categories for select using (true);
create policy if not exists model_ratings_select_anon on public.model_ratings for select using (true);
create policy if not exists model_combinations_select_anon on public.model_combinations for select using (true);
create policy if not exists user_model_preferences_select_auth on public.user_model_preferences for select to authenticated using (true);

-- Note: writes are restricted; use service role for ingestion jobs.
